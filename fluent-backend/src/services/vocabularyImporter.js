...
export function createVocabularyImporter({ dbService, vocabularyProcessor }) {
  ...
  const importer = {
    async importVocabulary(arr) {
      if (Array.isArray(arr)) {
        for (const element of arr) {
          try {
            const newWord = await importer.importWord(element);
            if (newWord?._id && Array.isArray(newWord.translations)) {
              await importer.setTranslationsAndTagsForTranslations(newWord);
            }
          } catch (error) {
    ...
    async setTranslationsAndTagsForTranslations(newWord) {
      const translations = [
        {
...
export const vocabularyImporter = createVocabularyImporter({ dbService, vocabularyProcessor });
